...
func (d *Device) Sqrt(aData *num.Data) *num.Data {
	...
	output.CalcGrad = func(ctx context.Context) {
		for i, y := range output.Data {
			Float32s(aData.Grad)[i] += output.Grad[i] * 0.5 / y
		}
	}
	return output
}
...
func (d *Device) MatrixMultiply2D(aData, bData *num.Data, alpha float32) *num.Data {
	...
	output.CalcData = func(ctx context.Context) {
		blas32.MatrixMultiplyAB(aW, aData.Data, bData.Data, output.Data, alpha, 0.0)
	}
	output.CalcGrad = func(ctx context.Context) {
		blas32.MatrixMultiplyAonTransposedB(oW, output.Grad, bData.Data, aData.Grad, alpha, 1)
		blas32.MatrixMultiplyTransposedAonB(oH, aData.Data, output.Grad, bData.Grad, alpha, 1)
	}
	return output
}
...
func (d *Device) MatrixMultiply3D(aData, bData *num.Data, alpha float32) *num.Data {
	...
	output.CalcData = func(ctx context.Context) {
		...
		for z := 0; z < oD; z++ {
			blas32.MatrixMultiplyAB(aData.Dims.W,
	...
	output.CalcGrad = func(ctx context.Context) {
		var ozOffset, izOffset, fzOffset int
		for z := 0; z < oD; z++ {
			blas32.MatrixMultiplyAonTransposedB(oW,
			...
			blas32.MatrixMultiplyTransposedAonB(aData.Dims.H,
				aData.Data[izOffset:izOffset+iWH],
				output.Grad[ozOffset:ozOffset+oWH],
