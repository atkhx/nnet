...
func (d *Device) NewData(dims num.Dims, srcNodes ...*Data) *Data {
	return &Data{
...
func (d *Device) newLinkedCopy(aData *Data) *Data {
	return d.NewData(aData.Dims, aData)
}

func (d *Device) NewDataRandNormWeighted(dims num.Dims, w float32) *Data {
	data := d.NewData(dims)
	data.Data.RandNormWeighted(w)
	return data
}

func (d *Device) NewTokenEmbeddingTable(featuresCount, alphabetSize int) *Data {
	data := d.NewData(num.NewDims(featuresCount, alphabetSize))
	data.Data.RandNorm()
	return data
}

func (d *Device) NewPositionEmbeddingTable(featuresCount, contextSize int) *Data {
...
func (d *Device) FillDataWithZeros(aData *Data) {
	aData.Data.Zero()
}

func (d *Device) FillDataWithOnes(aData *Data) {
	aData.Data.Ones()
}

func (d *Device) GetDataDims(aData *Data) num.Dims {
	return aData.Dims
}

func (d *Device) GetDataLength(aData *Data) int {
	return len(aData.Data)
}

func (d *Device) Sqrt(aData *Data) *Data {
...
func (d *Device) Mean(aData *Data) *Data {
...
func (d *Device) MeanByRows(aData *Data) *Data {
...
func (d *Device) VarianceByRows(aData, mean *Data) *Data {
	chunkSize := aData.Dims.W
	chunksCount := len(aData.Data) / chunkSize
	...
	output.calcData = func(ctx context.Context) {
		for i := 0; i < chunksCount; i++ {
			output.Data[i] = aData.Data[i*chunkSize : (i+1)*chunkSize].Variance(mean.Data[i])
		}
	}
	output.calcGrad = func(ctx context.Context) {
...
func (d *Device) LNorm(aData, gamma, beta *Data) *Data {
...
func (d *Device) Relu(aData *Data) *Data {
	output := d.newLinkedCopy(aData)
	output.calcData = func(ctx context.Context) {
...
func (d *Device) AddScalar(aData *Data, k float32) *Data {
	output := d.newLinkedCopy(aData)
	output.calcData = func(ctx context.Context) {
		aData.Data.AddScalarTo(output.Data, k)
	}
	output.calcGrad = func(ctx context.Context) {
...
func (d *Device) MulScalar(aData *Data, k float32) *Data {
	output := d.newLinkedCopy(aData)
	output.calcData = func(ctx context.Context) {
		aData.Data.MulScalarTo(output.Data, k)
	}
	output.calcGrad = func(ctx context.Context) {
...
func (d *Device) Add(aData, bData *Data) *Data {
...
func (d *Device) Sub(aData, bData *Data) *Data {
	config := broadcast.NewConfig(aData.Dims, bData.Dims)
	output := d.NewData(config.OutDims, aData, bData)
	output.calcData = func(ctx context.Context) {
		config.Broadcast(func(ax, bx, offset int) {
...
func (d *Device) Mul(aData, bData *Data) *Data {
	config := broadcast.NewConfig(aData.Dims, bData.Dims)
	output := d.NewData(config.OutDims, aData, bData)
	output.calcData = func(ctx context.Context) {
		config.Broadcast(func(ax, bx, offset int) {
...
func (d *Device) Div(aData, bData *Data) *Data {
	config := broadcast.NewConfig(aData.Dims, bData.Dims)
	output := d.NewData(config.OutDims, aData, bData)
	output.calcData = func(ctx context.Context) {
		config.Broadcast(func(ax, bx, offset int) {
...
func (d *Device) ConcatByRows(bData ...*Data) *Data {
	dims := bData[0].Dims
	...
	output.calcData = func(ctx context.Context) {
		...
		for i := 0; i < dims.H*dims.D; i++ {
			for _, node := range bData {
				output.Data[oOffset : oOffset+dims.W].CopyFrom(node.Data[bOffset : bOffset+dims.W])
				oOffset += dims.W
			}
			bOffset += dims.W
		}
	...
	output.calcGrad = func(ctx context.Context) {
		...
		for i := 0; i < dims.H*dims.D; i++ {
			for _, node := range bData {
				node.Grad[bOffset : bOffset+dims.W].Add(output.Grad[oOffset : oOffset+dims.W])
				oOffset += dims.W
			}
			bOffset += dims.W
		}
...
func (d *Device) Dropout(aData *Data, prob float32) *Data {
	if prob == 0 {
		return aData
	}
...
func (d *Device) Reshape(aData *Data, dims num.Dims) *Data {
...
func (d *Device) CrossEntropyPos(aData, targets *Data) *Data {
	if targets.Dims.W != 1 {
...
func (d *Device) Embeddings(aData *Data, tEmbeddings, pEmbeddings *Data) *Data {
	...
	output.calcData = func(ctx context.Context) {
		...
		for i, s := range aData.Data.ToInt() {
			...
			tFeatures.AddTo(outBuffer, pFeatures)
...
func (d *Device) Transpose(aData *Data) *Data {
	...
	output.calcData = func(ctx context.Context) {
		aData.Data.TransposeTo(output.Data, IW, IH)
	}

	output.calcGrad = func(ctx context.Context) {
		output.Grad.TransposeAndAddTo(aData.Grad, IH, IW)
	}
	return output
}

func (d *Device) TriangleLowerSoftmax(aData *Data) *Data {
...
func (d *Device) MatrixMultiply2D(aData, bData *Data, options ...num.MMOption) *Data {
	...
	output.calcData = func(ctx context.Context) {
		aData.Data.MatrixMultiply2DTo(bData.Data, output.Data, aW, alpha, 0.0)
		//blas.MatrixMultiplyAB(aW, aData.Data, bData.Data, output.Data, alpha, 0.0)
	}
	output.calcGrad = func(ctx context.Context) {
		output.Grad.MatrixMultiply2DAonTransposedBTo(bData.Data, aData.Grad, oW, alpha, 1)
		aData.Data.MatrixMultiply2DTransposedAonBTo(output.Grad, bData.Grad, oH, alpha, 1)
...
func (d *Device) MatrixMultiply(aData, bData *Data, options ...num.MMOption) *Data {
	if aData.Dims.W != bData.Dims.H {
		panic("aData width must be equal bData height")
	}
	...
	output.calcData = func(ctx context.Context) {
		...
		for z := 0; z < oD; z++ {
			aData.Data[izOffset:izOffset+iWH].MatrixMultiply2DTo(
	...
	output.calcGrad = func(ctx context.Context) {
		var ozOffset, izOffset, fzOffset int
		for z := 0; z < oD; z++ {
			output.Grad[ozOffset:ozOffset+oWH].MatrixMultiply2DAonTransposedBTo(
				bData.Data[fzOffset:fzOffset+fWH],
				aData.Grad[izOffset:izOffset+iWH],
				oW, alpha, 1,
			)

			aData.Data[izOffset:izOffset+iWH].MatrixMultiply2DTransposedAonBTo(
...
func (d *Device) GetOptimizerAdam(iterations int, beta1, beta2, learningRate, eps float32) func(nodes []*Data) func(iteration int) {
	iterations++
	return func(nodes []*Data) func(iteration int) {
